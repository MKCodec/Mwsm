...
run_step "$SUDO npm install -g npm@10 --quiet --no-progress" "Atualizando NPM" install
...
run_step "npm config set registry https://registry.npmjs.org >/dev/null 2>&1" "Configurando registro NPM" install
      ...
      if [[ "$DISTRO_DETECT" == "devuan" ]]; then
        run_step "$SUDO npm install -g npm@10 node-gyp@latest" "Atualizando npm e node-gyp" install
...
if [[ "$DISTRO_DETECT" == "devuan" ]]; then
    run_step "$SUDO npm install -g npm@10 node-gyp@latest --silent --no-audit --no-fund" "Atualizando npm e node-gyp" update
  else
    run_step "$SUDO npm install -g npm@10 --silent --no-audit --no-fund" "Atualizando npm" update
  fi
